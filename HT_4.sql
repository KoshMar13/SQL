DROP DATABASE IF EXISTS ht_4;
CREATE DATABASE ht_4;
USE ht_4;
...
SELECT 'males' as 'Gender', count(user_id) as "Likes count"
FROM likes
WHERE user_id in
(SELECT user_id FROM profiles WHERE gender = 'm') 
UNION
SELECT 'females', count(user_id)
FROM likes
WHERE user_id in
...
/*4. Пусть задан некоторый пользователь. Я выбираю пользователя 1. 
...
COUNT(id) OVER(Partition by from_user_id) as cm
FROM messages
...
ORDER BY cm DESC
LIMIT 1) AS ans);
-- Есть подозрения, что я пошёл абсолютно неоптимальным путём, 
-- но я бился над этой задачей целый день
...
JOIN friend_requests fr ON (fr.initiator_user_id = m.to_user_id AND
							fr.target_user_id = m.from_user_id)
                            OR (fr.target_user_id = m.to_user_id AND
